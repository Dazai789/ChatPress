...
下一阶段仍然围绕这条链路打磨：列表分页、搜索、状态筛选和后台列表页已经补齐，后续可以继续优化后台详情和编辑体验。
...
- 后台管理页面还只有 artifact 列表，暂时没有详情和编辑页面。
- 没有登录、鉴权和用户隔离。
...
```text
1. 从后台列表进入单个 artifact 详情。
2. 展示完整 sourceContent 和 renderedHtml 预览。
3. 支持修改 draft / published 状态。
4. 保留公开页面入口。
```
